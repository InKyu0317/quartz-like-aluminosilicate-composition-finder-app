...
- **열물성 예측**: Tg, Tx, Tliq, CTE, ΔT = Tx − Tg (단위: °C)
- **quartz 유사도 스코어**: tan δ 기준 정렬
- **UI 필터**: 알칼리 포함/제외, ε_r 범위, 최대 산화물 수, P(glass) 하한, SiO₂ 최소, Al₂O₃ 2위 조건
- **배경 그라디언트 테이블**: score / tan δ / P(glass) / ε_r / ×quartz / n_oxides
- **Bayesian Optimization 정밀 탐색**: 랜덤 검색 상위 N개를 Seed 삼아 GP-BO(Expected Improvement)로 새로운 조성 탐색 → 수렴 곡선·개선 이력·산화물 범위 차트 제공
